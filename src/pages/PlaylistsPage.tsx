        ...
        <div className="flex flex-wrap gap-1.5 mb-3">
          {selectedVideos.map((v, idx) => (
            ...
            >
              ...
              <button type="button" onClick={() => onRemove(v.id)} className="ml-1 hover:opacity-70" style={{ color: '#ff8a8a' }}>✕</button>
            </div>
          ))}
        </div>
      )}
      <div className="flex gap-2 mb-2">
        <input type="text" placeholder="영상 검색..." value={search} onChange={e => setSearch(e.target.value)} style={inputStyle} className="flex-1 min-w-0 px-3 py-1.5 rounded-lg text-sm focus:outline-none" />
        <input type="text" placeholder="태그" value={tagFilter} onChange={e => setTagFilter(e.target.value)} style={inputStyle} className="w-20 flex-shrink-0 px-3 py-1.5 rounded-lg text-sm focus:outline-none" />
      </div>
      <div className="rounded-xl overflow-hidden overflow-y-auto max-h-52" style={{ border: '1px solid var(--border)' }}>
        {videos.length === 0 ? (
          ...
          videos.map(video => {
            ...
            return (
              <button
                ...
                style={{ backgroundColor: isSelected ? 'var(--bg-card)' : 'transparent', opacity: isSelected ? 0.5 : 1, cursor: isSelected ? 'default' : 'pointer' }}
...
const PlaylistDetail: React.FC<{ id: string; onBack: () => void }> = ({ id, onBack }) => {
  ...
  const handleAddTag = async (tagName: string) => { await addTagToPlaylist({ variables: { playlistId: id, tagName } }); refetch(); };
  const handleRemoveTag = async (tagName: string) => { await removeTagFromPlaylist({ variables: { playlistId: id, tagName } }); refetch(); };

  if (loading)
    return <div className="min-h-screen pt-14 flex items-center justify-center" style={{ backgroundColor: 'var(--bg-base)', color: 'var(--text-muted)' }}>불러오는 중...</div>;
  if (!playlist)
  ...
  return (
    <div className="min-h-screen pt-14" style={{ backgroundColor: 'var(--bg-base)' }}>
      <div className="max-w-screen-lg mx-auto px-4 py-6">
        <button onClick={onBack} className="flex items-center gap-1 text-sm mb-6 hover:opacity-70 transition-opacity" style={{ color: 'var(--text-secondary)' }}>
          ← 목록으로
        </button>
        <div className="mb-4">
          ...
          <TagEditor tags={playlist.tags} onAdd={handleAddTag} onRemove={isLoggedIn ? handleRemoveTag : undefined} compact={false} />
        </div>

        {/* 영상 추가 */}
        <div className="mb-4">
          {!showAddVideo ? (
            <button onClick={() => setShowAddVideo(true)} style={btnSecondary} className="px-4 py-2 rounded-lg text-sm hover:opacity-80 transition-opacity">
              + 영상 추가
            </button>
          ) : (
                ...
                {searchVideos.map(video => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {isInPlaylist && <span className="text-xs flex-shrink-0" style={{ color: 'var(--text-muted)' }}>이미 추가됨</span>}
                      {isSelected && !isInPlaylist && <span className="text-xs flex-shrink-0" style={{ color: 'var(--accent)' }}>✓ 선택됨</span>}
                    </button>
                  );
                })}
              </div>
...
const PlaylistCard: React.FC<{
  ...
  const handleAddTag = async (tagName: string) => { await addTagToPlaylist({ variables: { playlistId: playlist.id, tagName } }); onTagUpdate(); };
  const handleRemoveTag = async (tagName: string) => { await removeTagFromPlaylist({ variables: { playlistId: playlist.id, tagName } }); onTagUpdate(); };
  ...
  return (
    <div className="group cursor-pointer" onClick={onClick}>
      ...
      <TagEditor tags={playlist.tags} onAdd={handleAddTag} onRemove={isLoggedIn ? handleRemoveTag : undefined} compact />

      {isLoggedIn && (
        <button onClick={e => { e.stopPropagation(); onDelete(playlist.id); }} className="text-xs mt-0.5 hover:opacity-70 transition-opacity" style={{ color: '#ff8a8a' }}>삭제</button>
      )}
    </div>
  );
};
    ...
    <div className="min-h-screen pt-14" style={{ backgroundColor: 'var(--bg-base)' }}>
      <div className="max-w-screen-2xl mx-auto px-4 pt-12 pb-6">
        ...
        <form onSubmit={handleSearch} className="mb-6 space-y-2">
          ...
          <div className="flex gap-2">
            <input
              ...
              style={inputStyle}
              className="flex-1 min-w-0 px-4 py-2 rounded-lg text-sm placeholder-gray-500 focus:outline-none"
            />
            <input
              type="text"
              placeholder="태그"
              ...
              style={inputStyle}
              className="w-24 flex-shrink-0 px-3 py-2 rounded-lg text-sm placeholder-gray-500 focus:outline-none"
            />
          </div>
          {/* 2행: 정렬 + 검색 + 생성 */}
          <div className="flex gap-2">
            <div className="relative flex-1 min-w-0">
              <select
                ...
                style={inputStyle}
                className="w-full px-3 py-2 pr-8 rounded-lg text-sm focus:outline-none appearance-none cursor-pointer"
              >
                <option value="VIEW_COUNT_DESC">인기순</option>
                <option value="CREATED_AT_DESC">최신순</option>
                <option value="NAME_ASC">이름순</option>
              </select>
              <span className="absolute right-2.5 top-1/2 -translate-y-1/2 pointer-events-none text-sm" style={{ color: 'var(--text-muted)' }}>▾</span>
            </div>
            <button type="submit" style={btnPrimary} className="flex-shrink-0 px-5 py-2 rounded-lg text-sm font-bold hover:opacity-80 transition-opacity">검색</button>
            <button type="button" onClick={() => setShowCreateModal(true)} style={btnSecondary} className="flex-shrink-0 px-4 py-2 rounded-lg text-sm font-medium hover:opacity-80 transition-opacity">+ 생성</button>
          </div>
        </form>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-40">
            <img src="/logo.png" alt="" className="w-[200px] sm:w-[400px] lg:w-[500px] h-auto mb-4 opacity-20 grayscale" />
          </div>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4">
        ...
        <Pagination page={page} hasNext={playlists.length >= 20} onChange={setPage} />
      </div>

      {/* 생성 모달: 모바일 bottom sheet */}
      {showCreateModal && (
        <div className="fixed inset-0 flex items-end sm:items-center justify-center z-50 p-0 sm:p-4" style={{ backgroundColor: 'rgba(0,0,0,0.7)' }}>
          <div className="rounded-t-2xl sm:rounded-2xl p-5 sm:p-6 w-full sm:max-w-lg max-h-[90vh] overflow-y-auto" style={{ backgroundColor: 'var(--bg-nav)', border: '1px solid var(--border)' }}>
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div>
                <label className="block text-sm mb-1" style={{ color: 'var(--text-secondary)' }}>
                  태그 <span className="text-xs ml-1" style={{ color: 'var(--text-muted)' }}>(입력하면 기존 태그 추천)</span>
                </label>
              ...
              <div>
                <label className="block text-sm mb-1" style={{ color: 'var(--text-secondary)' }}>
                  영상 추가 <span className="text-xs ml-1" style={{ color: 'var(--text-muted)' }}>(선택사항)</span>
                </label>
                <VideoSelector selectedVideos={selectedVideos} onSelect={v => setSelectedVideos(prev => [...prev, v])} onRemove={id => setSelectedVideos(prev => prev.filter(v => v.id !== id))} />
              </div>
              {createError && <p className="text-sm" style={{ color: '#ff8a8a' }}>{createError}</p>}
              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={() => { setShowCreateModal(false); setNewName(''); setNewTags(''); setSelectedVideos([]); }} className="px-4 py-2 text-sm hover:opacity-70 transition-opacity" style={{ color: 'var(--text-muted)' }}>취소</button>
