...
export const ReportPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen pt-14" style={{ backgroundColor: 'var(--bg-base)' }}>
      <div className="max-w-screen-md mx-auto px-4 pt-12 pb-6">

        {/* 헤더 */}
        <div className="flex items-center justify-between mb-6">
          <div>
            <h1 className="text-xl font-bold" style={{ color: 'var(--text-primary)' }}>신고 / 문의</h1>
            <p className="text-sm mt-0.5" style={{ color: 'var(--text-muted)' }}>문제가 있는 콘텐츠나 오류를 신고해 주세요</p>
          </div>
          <button onClick={() => setShowCreateModal(true)} style={btnPrimary} className="px-4 py-2 rounded-lg text-sm font-bold hover:opacity-80 transition-opacity">
            + 신고하기
          </button>
        </div>
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`px-2 py-0.5 rounded text-xs font-bold flex-shrink-0 ${report.isResolved ? '' : ''}`}
                        style={{
                    ...
                    <p className="text-xs line-clamp-1" style={{ color: 'var(--text-muted)' }}>{report.content}</p>
                  </div>
                  <div className="flex items-center gap-3 flex-shrink-0">
                    <span className="text-xs" style={{ color: 'var(--text-muted)' }}>{formatDate(report.createdAt)}</span>
          ...
          <div className="rounded-2xl p-6 w-full max-w-md" style={{ backgroundColor: 'var(--bg-nav)', border: '1px solid var(--border)' }}>
            <div className="flex items-center gap-2 mb-3">
              <span className="px-2 py-0.5 rounded text-xs font-bold"
                style={{ backgroundColor: detailTarget.isResolved ? '#1a4a2a' : '#4a2a1a', color: detailTarget.isResolved ? '#4ade80' : '#ffa07a' }}>
            ...
            <form onSubmit={handleCreate} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  required
                  placeholder="신고/문의 내용을 자세히 작성해 주세요"
                  rows={5}
                  style={inputStyle}
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={() => { setShowCreateModal(false); setNewTitle(''); setNewContent(''); }}
      ...
      {resolveTarget && (
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4" style={{ backgroundColor: 'rgba(0,0,0,0.7)' }}>
          <div className="rounded-2xl p-6 w-full max-w-md" style={{ backgroundColor: 'var(--bg-nav)', border: '1px solid var(--border)' }}>
            ...
            <p className="text-sm mb-4 line-clamp-1" style={{ color: 'var(--text-muted)' }}>{resolveTarget.title}</p>
            <div className="px-3 py-2 rounded-lg mb-4 text-sm" style={{ backgroundColor: 'var(--bg-card)', color: 'var(--text-secondary)' }}>
            ...
            <form onSubmit={handleResolve} className="space-y-4">
              ...
              <div className="flex justify-end gap-2 pt-2">
                <button type="button" onClick={() => { setResolveTarget(null); setAdminComment(''); }}
                  className="px-4 py-2 text-sm hover:opacity-70 transition-opacity" style={{ color: 'var(--text-muted)' }}>취소</button>
