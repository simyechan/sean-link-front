...
import type { PinballGameHandle, GamePhase } from './pinball.types'
...
export default function Pinball() {
  const gameRef = useRef<PinballGameHandle>(null)
  ...
  const isPlaying = phase === 'running' || phase === 'finished'
    ...
    <div className="flex h-screen pt-14 overflow-hidden">
      ...
      >
        ...
        <div className="flex flex-col gap-4 p-4 overflow-y-auto flex-1">
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="flex flex-col gap-1 max-h-48 overflow-y-auto">
              {names.length === 0 && (
                <p className="text-xs py-2 text-center" style={{ color: 'var(--text-muted)' }}>참가자를 추가해주세요</p>
              )}
              {filteredNames.length === 0 && search && (
                <p className="text-xs py-2 text-center" style={{ color: 'var(--text-muted)' }}>검색 결과 없음</p>
              )}
            ...
            {names.length > 0 && !isPlaying && (
              <div className="flex gap-3">
                <button onClick={() => { setNames([]); setSearch(''); }}
                  className="text-xs opacity-50 hover:opacity-100 transition"
                  style={{ color: 'var(--text-muted)' }}>전체 삭제</button>
              </div>
            )}
          </div>
          ...
          {/* 맵 */}
          <div className="flex flex-col gap-2">
            <label className="text-xs font-semibold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>맵</label>
            <div className="flex flex-col gap-1">
              {MAPS.map(map => (
                <button key={map.index} onClick={() => { setMapIndex(map.index); gameRef.current?.changeMap(map.index); }}
                  disabled={isPlaying}
                  className="px-3 py-2 rounded-lg text-sm text-left transition disabled:opacity-40"
                  style={{
          ...
          {/* 당첨 순위 */}
          <div className="flex flex-col gap-2">
            <label className="text-xs font-semibold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>당첨 순위</label>
            <div className="flex items-center gap-2">
              <button onClick={() => setWinningRank(r => Math.max(1, r - 1))} disabled={isPlaying}
                className="w-8 h-8 rounded-lg text-sm font-bold transition disabled:opacity-40 flex-shrink-0"
                style={{ backgroundColor: 'var(--bg-nav)', color: 'var(--text-primary)' }}>−</button>
              <input type="number" min={1} max={names.length || 1} value={winningRank}
                onChange={e => { const v = Number(e.target.value); if (v >= 1) setWinningRank(v); }}
                disabled={isPlaying}
                className="flex-1 text-center font-bold rounded-lg py-1 border disabled:opacity-40"
                style={{ backgroundColor: 'var(--bg-nav)', borderColor: 'var(--border)', color: 'var(--text-primary)' }} />
              <span className="text-sm flex-shrink-0" style={{ color: 'var(--text-muted)' }}>등</span>
              <button onClick={() => setWinningRank(r => Math.min(names.length || 1, r + 1))} disabled={isPlaying}
                className="w-8 h-8 rounded-lg text-sm font-bold transition disabled:opacity-40 flex-shrink-0"
                style={{ backgroundColor: 'var(--bg-nav)', color: 'var(--text-primary)' }}>+</button>
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            <label className="text-xs font-semibold uppercase tracking-wider" style={{ color: 'var(--text-muted)' }}>스킬 활성화</label>
            <button onClick={() => setUseSkills(v => !v)} disabled={isPlaying}
          ...
          {phase === 'idle' && (
            <p className="text-xs text-center py-1" style={{ color: 'var(--text-muted)' }}>
              참가자를 추가하면 미리보기가 시작됩니다
            </p>
          )}
          {phase === 'preview' && (
            <div className="flex flex-col gap-2">
              <button onClick={() => gameRef.current?.shuffle()}
                className="w-full py-2.5 rounded-lg font-bold text-sm transition border"
          ...
          {phase === 'running' && (
            <p className="text-xs text-center py-1 animate-pulse" style={{ color: 'var(--text-muted)' }}>
              진행 중...
            </p>
          )}
          {phase === 'finished' && (
            <button onClick={() => { gameRef.current?.reset(); setPhase('idle'); }}
              className="w-full py-3 rounded-lg font-bold text-sm transition"
        ...
        <div className="p-4 border-t flex-shrink-0" style={{ borderColor: 'var(--border)' }}>
          <a href="https://github.com/lazygyu/roulette" target="_blank" rel="noopener noreferrer"
      ...
      <main className="flex-1 overflow-hidden p-4">
        {names.length === 0 ? (
          <div className="w-full h-full flex flex-col items-center justify-center opacity-40">
            ...
            <p className="text-sm mt-3" style={{ color: 'var(--text-muted)' }}>참가자를 추가하면 미리보기가 시작됩니다</p>
          </div>
        ) : (
          <PinballGame
            ref={gameRef}
