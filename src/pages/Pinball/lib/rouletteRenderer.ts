...
export class RouletteRenderer {
  ...
  protected setupResizeObserver(resizing: (entries?: ResizeObserverEntry[]) => void) {
  ...
  async init() {
    ...
    this.setupResizeObserver(resizing);
  }
  ...
  const base = '/pinball-assets/assets/images/';
  const loadPromises = [
    { name: '챔루',      imgUrl: `${base}chamru.png` },
    { name: '쿠빈',      imgUrl: `${base}kubin.png` },
    { name: '꽉변',      imgUrl: `${base}kkwak.png` },
    { name: '꽉변호사',  imgUrl: `${base}kkwak.png` },
    { name: '꽉 변호사', imgUrl: `${base}kkwak.png` },
    { name: '주누피',    imgUrl: `${base}junyoop.png` },
    { name: '왈도쿤',    imgUrl: `${base}waldokun.png` },
  ].map(({ name, imgUrl }) =>
    (async () => { this._images[name] = await this._loadImage(imgUrl); })()
  );

  loadPromises.push(
    (async () => { await this._loadImage(`${base}ff.svg`); })()
  );
