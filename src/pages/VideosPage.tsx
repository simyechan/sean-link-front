...
const VideoCard: React.FC<{
  ...
  onVideoClick: (video: VideoModel, e: React.MouseEvent) => void;
  onTagUpdate: () => void;
  isLoggedIn: boolean;
}> = ({ video, onDelete, onVideoClick, onTagUpdate, isLoggedIn }) => {
  const [addTagToVideo] = useMutation(ADD_TAG_TO_VIDEO);
  const [removeTagFromVideo] = useMutation(REMOVE_TAG_FROM_VIDEO);

  const handleAddTag = async (tagName: string) => {
    await addTagToVideo({ variables: { videoId: video.id, tagName } });
    onTagUpdate();
  };

  const handleRemoveTag = async (tagName: string) => {
    await removeTagFromVideo({ variables: { videoId: video.id, tagName } });
    onTagUpdate();
  };
  ...
  return (
    <div className="group cursor-pointer transition-transform hover:-translate-y-1">
      ...
      >
        ...
        {video.duration && (
          <span
            className="absolute bottom-1 right-1 text-white text-xs px-1 py-0.5 rounded font-mono"
        ...
        {video.platform && (
          <span
            className="absolute top-1.5 left-1.5 text-white text-xs px-1.5 py-0.5 rounded font-bold"
            style={{
              backgroundColor: video.platform === 'CHZZK' ? '#1a8a4a' : '#cc2200',
            }}
          >
  ...
  return (
    <div className="min-h-screen pt-20" style={{ backgroundColor: 'var(--bg-base)' }}>
      <div className="max-w-screen-2xl mx-auto px-4 py-6">
        ...
        <div className="mb-6 space-y-2">
          ...
          <div className="flex gap-2">
            <input
              ...
              style={inputStyle}
              className="flex-1 min-w-0 px-4 py-2 rounded-lg text-sm focus:outline-none"
            />
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              style={btnPrimary}
              className="flex-shrink-0 px-5 py-2 rounded-lg text-sm font-bold hover:opacity-80 transition-opacity"
            >
              검색
            </button>
            <button
              ...
              style={btnSecondary}
              className="flex-shrink-0 px-4 py-2 rounded-lg text-sm hover:opacity-80 transition-opacity"
            >
              + 추가
            </button>
          </div>
        ...
        <Pagination page={page} hasNext={videos.length >= 20} onChange={setPage} />
      </div>
            ...
            <form onSubmit={handleAdd} className="space-y-4">
              ...
              <div>
                <label className="block text-sm mb-1" style={{ color: 'var(--text-secondary)' }}>
                  태그
                  <span className="ml-1 text-xs" style={{ color: 'var(--text-muted)' }}>(쉼표로 구분)</span>
                </label>
